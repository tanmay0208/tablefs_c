...
bool LevelDBIterator_Valid(LevelDBIterator *leveldbiterator) { return db_iter_valid( leveldbiterator->iter_ ) ; };

  void LevelDBIterator_SeekToFirst(LevelDBIterator *leveldbiterator) { db_iter_seek_to_first(leveldbiterator->iter_); }

  void LevelDBIterator_SeekToLast(LevelDBIterator *leveldbiterator) { db_iter_seek_to_last(leveldbiterator->iter_); }

  void LevelDBIterator_Seek(LevelDBIterator *leveldbiterator,Slice *target) { 
      db_iter_seek(leveldbiterator->iter_,target->data_,target->size_); 
  }

  void LevelDBIterator_Next(LevelDBIterator *leveldbiterator) { db_iter_next(leveldbiterator->iter_); }

  void LevelDBIterator_Prev(LevelDBIterator *leveldbiterator) { db_iter_prev(leveldbiterator->iter_); } 
...
int LevelDBAdaptor_Init(LevelDBAdaptor *leveldbadaptor) {
 ...
 //leveldbadaptor->cache_ = (F_cache_size >= 0) ? db_cache_create_lru(F_cache_size) : NULL;
  db_name = Properties_getProperty_default(leveldbadaptor->p_,"leveldb.db",(char*) "/tmp/db");
  leveldb_options_t *options;
  //options= db_options_create() ;
  db_options_set_create_if_missing(options,Properties_getPropertyBool(leveldbadaptor->p_,"leveldb.create.if.missing.db", true));
  db_options_set_cache(options,leveldbadaptor->cache_);
  db_options_set_block_size(options,Properties_getPropertyInt(leveldbadaptor->p_,"leveldb.block.size", 4 << 10));
  db_options_set_write_buffer_size(options,Properties_getPropertyInt(leveldbadaptor->p_,"leveldb.write.buffer.size", 16<<20));
  db_options_set_max_open_files(options,Properties_getPropertyInt(leveldbadaptor->p_,"leveldb.max.open.files", 800));
  db_options_set_filter_policy(options, leveldb_filterpolicy_create_bloom(12));
...
LevelDBIterator* LevelDBAdaptor_GetNewIterator(LevelDBAdaptor *leveldbadaptor) {
  ...
  //leveldb_iterator_t *iter = db_create_iterator(leveldbadaptor->db_,read_options);
  leveldb_iterator_t *iter;     //added for compiling
...
int LevelDBAdaptor_Sync(LevelDBAdaptor *leveldbadaptor) {
  ...
  db_writeoptions_set_sync(write_options,true);
